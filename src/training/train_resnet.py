"""Train the residual Lorenz predictor."""
...
def main() -> None:
    parser = build_config_argparser("Train the Lorenz residual baseline.")
    ...
    model = build_model("resnet", model_kwargs).to(device)
    ...
    summary = train_supervised_model(
        ...
        run_name="resnet",
        extra_checkpoint_data={"model_name": "resnet", "model_kwargs": model_kwargs},
    )
    save_config_snapshot(config, checkpoint_dir, filename="resnet_config_used.yaml")
    ...
    figure_path = figure_dir / "resnet_validation_rollout.png"
    plot_time_series_comparison(
        time_grid[history_steps : history_steps + rollout_horizon],
        target,
        prediction,
        str(figure_path),
        title="Residual model validation rollout",
    )

    summary["sample_rollout_figure"] = str(figure_path)
    save_json(summary, checkpoint_dir / "resnet_train_summary.json")
